...
- [x] Jackson discovery triage — DONE (2026-06-08). Reviewed all 91 findings in pending_sources_v3.json. Added 2 clean WordPress Tribe API sources to TRIBE_EVENT_SOURCES: Snow King Mountain (+22, King Concert series + mountain events) and Dishing Jackson Hole (+91, Food & Drink). Jackson in-radius 1283→1379. Rejected the rest: already-wired (Cloudveil, Wildlife Art, Chamber, GTMF, CFA, JHMR, Cowboy Bar), or quality_score×future_ratio ≈ 0 (all-past or non-event domains: travelwyoming, mentalwellnessjh, velo22, jhpublicart, jhlandtrust, rmrentals, mountainmodern, skinnyskis, findarace, astoria, etc.). High raw event_url_count is misleading without future_ratio — rank by quality×future_ratio, not count. Don't re-triage the same 91.
