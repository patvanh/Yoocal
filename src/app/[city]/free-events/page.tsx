...
export async function generateMetadata(
  ...
  const url = `https://www.yoocal.com/${city}/free-events`;
  return {
    title: `Free Events in ${cfg.label} \u2014 Free Things to Do | Yoocal`,
    description: `Free things to do in ${cfg.label} \u2014 free concerts, festivals, markets, and family events. Updated daily, no admission required.`,
    alternates: { canonical: url },
    openGraph: {
      title: `Free Events in ${cfg.label}`,
      description: `Free things to do in ${cfg.label} \u2014 updated daily.`,
      url,
      type: "website",
    },
  };
...
export default async function CityFreeEventsPage(
  { params }: { params: Promise<{ city: string }> },
) {
  const { city } = await params;
  const cityKey = cityKeyFromSlug(city);
    ...
    <>
      ...
      <div className="hero">
        <div className="hero-content">
          ...
          <h1>Free things to do in <em>{cfg.label}.</em></h1>
          <p>
            {events.length > 0
              ? `${events.length} free upcoming events \u2014 no admission required.`
      ...
      <div className="content">
        {events.length === 0 ? (
          <div className="empty">
            ...
            <h2>No free events on file right now</h2>
            <p>Our scraper runs daily \u2014 check back soon, or browse the full <a href={`/${city}`}>{cfg.label} calendar</a>.</p>
          </div>
        ) : (
          <IntentDayList events={events} variant="columns" citySlug={city} />
        )}
